...
async def send_invite_whatsapp(phone: str, visitor_name: str, otp: str, qr_code: Optional[str] = None, visit_id: Optional[UUID] = None) -> bool:
    """
    ...
    # Build pass link: prefer /pass/{visit_id} (QR-only flow), fall back to /qr/{qr_code}
    pass_link: Optional[str] = None
    base = (settings.FRONTEND_BASE_URL or "").rstrip("/")
    if base:
        if visit_id:
            pass_link = f"{base}/pass/{visit_id}"
        elif qr_code:
            pass_link = f"{base}/qr/{qr_code}"

    lines = [f"Visitor pass: {visitor_name}"]
    if pass_link:
        lines.append(f"Open your visitor pass:\n{pass_link}")
    if otp:
        lines.append(f"Backup code (if needed): {otp}")
