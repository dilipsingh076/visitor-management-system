...
import { isAuthenticated, getCachedUser, getCurrentUser, canAccessPlatform } from "@/lib/auth";
import { listSocieties, createSociety, type SocietyListItem } from "@/lib/api";
...
export default function PlatformSocietiesPage() {
  ...
  useEffect(() => {
    if (!user || !canAccessPlatform(user)) return;
    listSocieties().then(setSocieties);
  }, [user]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const created = await createSociety({
      ...
      slug: createSlug.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "society",
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {creating && (
        <form
          ...
          className="mb-8 p-6 bg-card border border-border rounded-xl space-y-4"
        >
          <h2 className="text-lg font-semibold text-foreground">Create society</h2>
          {createError && (
            <p className="text-sm text-error bg-error-light px-3 py-2 rounded-lg">{createError}</p>
          )}
          <div>
          ...
          <Button type="submit" variant="primary" size="md" disabled={submitLoading}>
