...
import { useBuildings, useInviteVisitor, type InviteVisitorResult } from "@/features/visitors";
...
const QRCodeSVG = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="h-[220px] w-[220px] bg-muted rounded animate-pulse" />,
  }
);

export function InviteVisitorPageContent() {
    ...
    <PageWrapper width="narrower">
      ...
      {result ? (
        <div className={`${theme.surface.card} p-5`}>
          ...
          {result.otp && (
            <p className="text-xs text-muted-foreground text-center mb-4">Backup code: {result.otp}</p>
          )}
