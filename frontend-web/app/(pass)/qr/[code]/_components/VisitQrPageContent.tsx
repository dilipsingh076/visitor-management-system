...
const QRCodeSVG = dynamic(
  () => import("qrcode.react").then((mod) => mod.QRCodeSVG),
  {
    ssr: false,
    loading: () => (
      <div className="h-[240px] w-[240px] mx-auto bg-muted-bg rounded-lg animate-pulse" />
    ),
  }
);

export function VisitQrPageContent() {
  ...
  const handleSystemShare = async () => {
    if (!shareUrl || typeof navigator === "undefined" || !(navigator as unknown as { share?: unknown }).share) {
      setShareError("System share is not available on this device.");
      return;
    }
    setShareError(null);
    try {
      await (navigator as unknown as { share: (opts: object) => Promise<void> }).share({
